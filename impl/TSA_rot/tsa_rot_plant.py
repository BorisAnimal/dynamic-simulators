...
class TSARotDynamic:
    ...
    def __call__(self, state, t, u):
        ...
        # Control
        u = 0.0
        # ddtheta = 1/(I + m*J**2*R**3/r) * (u - (b_theta*dtheta + J*R*(R**2/r*m*dJ*dtheta + m*g*R/r*np.sin(alpha) + b_alpha*dalpha)))
        ddtheta = 1 / (I + J ** 2 * m * R / r) * (
                u - J * (m * R / r * (dJ * dtheta + g) + b_alpha * dalpha) - b_theta * dtheta)
        return np.array([dtheta, ddtheta, ])


class TSARotAsyncPlant(AsyncSimulator):
    def __init__(self, x0, t, data_file='tsa_results.pkl', **kwargs):
        super().__init__(x0=x0, t=t, data_file=data_file, **kwargs)
        # specified params
        self._dynamic = TSARotDynamic(**kwargs)
...
if __name__ == '__main__':
    ...
    ## Setup environment
    plant = TSARotAsyncPlant(x0=[0.0, 100.0], t=3)
