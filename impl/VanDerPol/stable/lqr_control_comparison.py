...
if __name__ == '__main__':
    ...
    dynamic = VanderpolDynamic(stable_zero=True)
    ...
    u0_lift = - np.linalg.pinv(Blift).dot(
        (np.eye(*Alift.shape) - Alift).dot(x0_lift)
    ).flatten()  # [0]
    # u0_lift = np.linalg.pinv(Blift).dot(Alift.dot(x0_lift)).flatten()[0]
    # u0_lift = 0.0
    Q = 1e1 * np.eye(Alift.shape[0])
    Q[0, 0] = 1e4
    Q[1, 1] = 1e3
    controller = LQR(
        x0, Alift, Blift, u0=u0_lift,
        Q=Q,
        R=np.diag([1e3]),
        Umax=3.0, lift=lift
    )
